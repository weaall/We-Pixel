...
export async function buildPackage(o: PackageOptions): Promise<PackageResult> {
  ...
  const add = (path: string, content: string | Blob) => {
    zip.file(path, content)
    entries.push(path)
  }

  // 1. 에셋 본체 — 반드시 1배율이어야 한다.
  const png = await docToPngBlob(o.doc, 1)
  ...
  if (o.previewScale > 1) {
    const preview = await docToPngBlob(o.doc, o.previewScale)
  ...
  return { blob, filename: `WePixel_${assetName}.zip`, entries, warnings }
}
