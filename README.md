...
- *스프라이트*: 타원형 핵을 보장하고 그 바깥 테두리만 펄린 노이즈로 흔듭니다.
  전체를 노이즈에 맡기면 형태가 조각나고, 가장 큰 조각이 하필 세로로 길면
  "납작"을 골랐는데 길쭉한 결과가 나옵니다. 체형(보통·길쭉·납작)을 고를 수 있습니다.
- 명암은 네 방향 경계를 가중치로 더해 다섯 단계로 나눕니다. 상단만 밝게 칠하면
  테두리만 두 톤인 납작한 그림이 됩니다.
- 팔레트는 밝은 쪽을 따뜻하게, 어두운 쪽을 차갑게 색조를 옮깁니다. 명도만 바꾼
  램프는 회색을 섞은 것처럼 탁해 보입니다.
