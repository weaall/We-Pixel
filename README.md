...
- *주사위*: 등축 큐브를 절차적으로 그립니다. 색조와 재질(돌·금속·나무·보석)만
  바꾸면 같은 형태로 다른 주사위가 나옵니다 — 세트를 만들 때 쓰는 방식입니다.

  세 면이 모두 평행사변형이라 한 가지 방법으로 다룹니다. 점을 두 모서리 벡터의
  선형결합으로 풀면 면 안에 있는지와 면 안에서의 좌표를 한 번에 얻습니다 —
  눈 위치와 표면 잡티가 그 좌표를 그대로 씁니다.

  마주보는 면의 합은 7이므로 보이는 세 면은 (1,6) (2,5) (3,4)에서 하나씩입니다.
  같은 눈이 두 번 보이거나 1과 6이 함께 보이는 일은 없습니다.
- *색 변형*: 지금 캔버스에 있는 그림의 색만 바꿔 여러 벌을 한 번에 뽑습니다.
  같은 주사위의 다른 색, 같은 갑옷의 다른 등급처럼 **이미 그린 것으로 세트를
  만들 때** 씁니다. 앞의 세 가지가 형태를 만들어 내는 쪽이라면 이쪽은 가진
  형태를 지키는 쪽입니다.
...
| **가상 생성** | 배색 여러 벌을 한 번에 (자동 생성 > 색 변형) | **모양 100% 고정** |
...
| `generate_dice` | 등축 주사위 생성 (색조·재질·눈) |
